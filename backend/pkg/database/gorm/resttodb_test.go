...
func TestConvertToDBScanConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ConvertToDBScanConfig(tt.args.config)
			if (err != nil) != tt.wantErr {
				t.Errorf("ConvertToDBScanConfig() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ConvertToDBScanConfig() got = %v, want %v", got, tt.want)
			}
		})
	}
}
