...
func initDataBase(config types.DBConfig) (*gorm.DB, error) {
	...
	// that.
	idb = db.Exec(fmt.Sprintf("CREATE INDEX IF NOT EXISTS asset_scans_findings_processed_idx ON asset_scans((%s), (%s))", SQLVariant.JSONExtract("Data", "$.findingsProcessed"), SQLVariant.JSONExtract("Data", "$.generalStatus.state")))
	if idb.Error != nil {
