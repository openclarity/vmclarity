...
func (w *Watcher) getLatestAssetScanStats(ctx context.Context, asset *models.Asset) models.AssetScanStats {
	...
	filterTmpl := "asset/id eq '%s' and generalStatus/state eq 'Done' and (status/%s/errors eq null or length(status/%s/errors) eq 0) and scanFamiliesConfig/%s/enabled eq true"
	...
	for _, family := range families {
		params := models.GetAssetScansParams{
			...
			OrderBy: utils.PointerTo("generalStatus/lastTransitionTime DESC"),
		}
