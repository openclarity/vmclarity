...
func Test_parseSSHKeyGenCommandOutput(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := parseSSHKeyGenCommandOutput(tt.args.output, tt.args.infoType, tt.args.path); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("parseSSHKeyGenCommandOutput() = %v, want %v", got, tt.want)
			}
		})
	}
}
