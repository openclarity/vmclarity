...
const WidgetContent = ({ data }) => {
  ...
  const formattedData = orderBy(
    ...
    ["desc"],
  ).map(({ regionName, findingsCount }) => ({ regionName, ...findingsCount }));
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart
          ...
          layout="vertical"
          barSize={10}
          margin={{ top: 12, right: 10, left: 20, bottom: 60 }}
        >
          <CartesianGrid
            horizontal={false}
            style={{ stroke: COLORS["color-grey-lighter"] }}
          />
          <XAxis
            type="number"
            tick={{ fill: COLORS["color-grey"] }}
            style={{ fontSize: "12px" }}
          />
          <YAxis
            type="category"
            dataKey="regionName"
            tick={{ fill: COLORS["color-grey-black"] }}
            style={{ fontSize: "12px" }}
          />
          <Tooltip
            content={(props) => (
              <ChartTooltip
                {...props}
                widgetFindings={WIDGET_FINDINGS_ITEMS}
                headerDisplay={TooltipHeader}
                countKeyName="dataKey"
              />
            )}
            wrapperStyle={{
          ...
          />
          {WIDGET_FINDINGS_ITEMS.map(
            ({ dataKey, color }) =>
              selectedFilters.includes(dataKey) && (
                <Bar
                  key={dataKey}
                  dataKey={dataKey}
                  stackId={BAR_STACK_ID}
                  fill={color}
                />
              ),
          )}
