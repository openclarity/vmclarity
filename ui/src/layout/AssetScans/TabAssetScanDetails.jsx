...
const TabAssetScanDetails = ({ data }) => {
  ...
  return (
    <DoublePaneDisplay
      ...
      leftPaneDisplay={() => (
        <>
          <Title medium onClick={() => navigate(`${ROUTES.ASSETS}/${assetId}`)}>
            Asset
          </Title>
          <TitleValueDisplayRow>
          ...
          <Title
            medium
            onClick={() =>
              navigate(`${ROUTES.SCANS}/${SCANS_PATHS.SCANS}/${scanId}`)
            }
          >
            Scan
          </Title>
          ...
          {STATUS_DISPLAY_ITEMS.map(({ dataKey, title }, index) => {
            ...
            return (
              <div key={dataKey} style={{ marginTop: ITEM_MARGIN }}>
                <TitleValueDisplay
                  title={!!title ? `${title} scan details` : null}
                  isLargeTitle
                >
                  <div style={{ marginBottom: ITEM_MARGIN }}>
                    <StatusDisplay
                      state={state}
                      errors={message === undefined ? [] : [message]}
                    />
                  ...
                  {typeStats?.map((typeStats, index) => (
                    <StatsDisplay key={index} {...typeStats} />
                  ))}
                </TitleValueDisplay>
