...
func CreateExporter() (*Exporter, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("unable to create VMClarity API client. server address=%v: %w", server, err)
	}
...
func (e *Exporter) MarkScanResultInProgress() error {
	scanResult, err := e.client.GetScanResult(scanResultID)
	if err != nil {
		return err
	}
	...
	err = e.client.PatchScanResult(scanResult, scanResultID)
	if err != nil {
		return err
	}
