...
func (c *CLI) MountVolumes(ctx context.Context) ([]string, error) {
	...
	for _, disk := range blockInfo.Disks {
		for _, part := range disk.Partitions {
			...
			if err := mount.Mount(ctx, part.Name, mountPoint, part.FilesystemLabel, DefaultMountOptions); err != nil {
				return nil, fmt.Errorf("failed to mount partition %s at %s: %w", part.Name, mountPoint, err)
			}
