...
func (p *Provider) createScanContainer(ctx context.Context, assetScanMount *mount.Mount, networkID string, config *provider.ScanJobConfig) (string, error) {
	...
	images, err := p.dockerClient.ImageList(ctx, imagetypes.ListOptions{
	...
	if len(images) == 0 {
		imagePullResp, err := p.dockerClient.ImagePull(ctx, config.ScannerImage, imagetypes.PullOptions{})
		if err != nil {
