...
func getScanName(config *provider.ScanJobConfig) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to get scan name. Provider=%s: %w", models.Docker, err)
	}
...
func getAssetId(config *provider.ScanJobConfig) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to get asset object type. Provider=%s: %w", models.Docker, err)
	}
	...
	case "ContainerInfo":
		...
		if err != nil {
			return "", fmt.Errorf("failed to get asset id. Provider=%s: %w", models.Docker, err)
		}
	...
	default:
		return "", fmt.Errorf("get asset id not implemented for current object type (%s). Provider=%s", models.Docker, objectType)
	}
}
...
func getAssetName(config *provider.ScanJobConfig) (string, error) {
	...
	case "ContainerInfo":
		containerInfo, err := config.AssetInfo.AsContainerInfo()
		if err != nil {
			return "", fmt.Errorf("failed to get asset name. Provider=%s: %w", models.Docker, err)
		}
